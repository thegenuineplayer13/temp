...
export function ScrollActionPanel({
  ...
  const getPositionClasses = () => {
    if (isAtBottom) {
      return "relative bottom-0 left-0 right-0 w-full";
    }
  ...
  if (isMobile) {
    return (
      <>
        <div ref={panelRef} className={cn(getPositionClasses(), "z-30 transition-all duration-300", className)}>
          {isAtBottom ? (
        ...
        {fabOpen && !isAtBottom && (
          <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-20" onClick={() => setFabOpen(false)} />
        )}
      </>
    );
  }
